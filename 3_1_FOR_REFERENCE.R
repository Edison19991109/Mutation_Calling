...
#Check the reference genome to see which species.
...
#Check the environment to get the script path    
...
#It assumes that the filter_functions.R script is in the same directory as FILTER_AFTER_ANNOVAR
##filter_functions.R is a dependent script.
source(paste(scriptPath,"/somatic_script/filter_functions.R",sep=""))

##It assumes that the cancer_gene_census.csv is in the same directory as FILTER_AFTER_ANNOVAR
cosmic_genes=read.csv(paste(scriptPath,"/somatic_script/cancer_gene_census.csv",sep=""),row.names=1,
                      stringsAsFactors = F)

#These genes usually turn out to have somatic mutations in 
#any cohort of patients
...
#Check if there is a output file directory.
if (!file.exists(path)) {dir.create(path)}

########  check parental origin (for sample mislabeling)  ##################

#If the pateint ID is not available, then set the patient_id as sample_id.
...
#More detail filtering of ANNOAR results.
mutations=c()
for (i in 1:dim(design)[1])
{
  # read data
  file=design$folder[i]
  tmp=read.table(file,stringsAsFactors = F,sep="\t",header = T,
                 colClasses=c("Ref"="character","Alt"="character"))
  if (dim(tmp)[1]==0) {next}
